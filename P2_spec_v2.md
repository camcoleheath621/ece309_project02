...
In Project 2, you will build the core execution loop (`miniharness`) from scratch in C++. 
...
~~~
* **C++ Name Hiding:** This uses the Non-Virtual Interface (NVI) idiom. When you subclass `ModelClient` (e.g., for `ScriptedModelClient`) and override the virtual `generate` function, C++ will hide the non-virtual version. You must use `using ModelClient::generate;` in your derived classes to prevent compiler errors (New to abstract classes and virtual functions? See Appendix B).
...
`Harness::run` reads user turns from an `InputSource` and writes output to an `OutputSink` — abstractions over where turns come from and where output goes (the terminal in normal use, something else in your tests). You must provide at least one concrete implementation of each so `miniharness` can run interactively.
...
## 8. Appendix B — Primer: Abstract Base Classes and Virtual Functions
